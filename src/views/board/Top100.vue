    ...
    <div class="content1">
      ...
      <dl>
        <dd v-for="(item,index) in filmPageInfo.filmList" :key="index">
          <div>
            <i :class="'No'+((index+1)+(pageData.currentPage-1)*pageData.pageSize)">{{ (index+1)+(pageData.currentPage-1)*pageData.pageSize }}</i>
          </div>
          <div class="img">
            <router-link :to="{path:'/FilmDetail',query:{filmId:item._id}}">
              <img :src="'http://localhost:3000/'+item.filmImg" alt="">
            </router-link>
          </div>
          <div>
            <div>
              <router-link :to="{path:'/FilmDetail',query:{filmId:item._id}}" :title=item.filmName class="font">
                {{ item.filmName }}
              </router-link>
              <p>
                导演:{{ item.director }}
...
export default {
  ...
  methods:{
    ...
    //正在热映
    async queryManyFilm() {
      await this.queryManyFilmAsync({...this.pageData,status:1,viewCount:-1});
    },
    test(val){
