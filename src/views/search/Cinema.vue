      ...
      <div class="content1">
        ...
        <dl v-if="cinemaList.length!=0">
          <dd v-for="(item,index) in cinemaList" :key="index" class="" >
            ...
            <div class="cinema">
              <router-link :to="{path:'/CinemaDetail',query:{roomId:item._id}}">
                <img :src="'http://localhost:3000/'+item['brand'][0].brandImg" alt="">
              </router-link>
            </div>
            <div>
              <div>
                <router-link :to="{path:'/CinemaDetail',query:{roomId:item._id}}" :title=item.cinemaName class="font">
                  {{ item['brand'][0].brandName }}
                </router-link>
                <p>
                  影城名称:{{ item['brand'][0].brandName }}
                </p>
                <p>
                  影城地址：{{ item['cinema'][0].cinemaDistrict + item['cinema'][0].cinemaAddress }}
                </p>
