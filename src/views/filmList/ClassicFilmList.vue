      ...
      <div class="movies-channel">
          ...
          <ul class="tags-lines">
            <li class="tags-line">
              ...
              <ul class="tags" id="movieTypeId">
                <li @click="searchFilmType()" :class="searchInFo.filmType?'':'active'">
                  <a href="javascript:void(0);" style="cursor: pointer">全部</a>
                </li>
                <li v-for="(type,index) in filmType" :key="index"  @click="searchFilmType(type._id)" :class="searchInFo.filmType==type._id?'active':''">
            ...
            <li class="tags-line tags-line-border">
              ...
              <ul class="tags" id="movieAreaId">
                <li @click="searchFilmArea()" :class="searchInFo.filmArea?'':'active'">
                  <a href="javascript:void(0);" style="cursor: pointer">全部</a>
                </li>
                <li v-for="(area,index) in filmArea" :key="index"  @click="searchFilmArea(area)" :class="searchInFo.filmArea==area?'active':''">
            ...
            <li class="tags-line tags-line-border">
              ...
              <ul class="tags" id="movieTimeId">
                <li @click="searchFilmLanguage()"  :class="searchInFo.language?'':'active'">
                  <a href="javascript:void(0);" style="cursor: pointer">全部</a>
                </li>
                <li  v-for="(language,index) in filmLanguage" :key="index"  @click="searchFilmLanguage(language)" :class="searchInFo.language==language?'active':''">
        ...
        <div class="movies-panel">
          ...
          <div class="movies-list">
            <dl class="movie-list" v-if="getFilmListLength(classicPageInfo.filmList)" id="movieListId">
              <dd v-for="(film,index) in classicPageInfo.filmList" :key="index" >
                <div class="movie-item film-channel">
                  <router-link :to="{path:'/filmDetail',query:{filmId:film._id}}" target="_blank">
...
<!--                      </el-image>-->
                ...
                <div class="channel-detail movie-item-title">
                  <router-link to="/FilmDetail" target="_blank">{{film.filmName}}</router-link>
                </div>
...
export default {
  ...
  data(){
    return{
      ...
      filmArea:["大陆","美国","韩国","日本","泰国","印度","法国","英国","俄罗斯","西班牙","德国","意大利","澳大利亚","伊朗","其他"],
  ...
  methods:{
    ...
    queryManyFilm(){
      this.queryManyFilmAsync({...this.pageData,...this.searchInFo,...this.status,...this.filmSort});
    },
    searchFilmType(val){
